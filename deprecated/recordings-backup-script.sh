...
LOGFILE="/volume1/backups/audio/audio_recordings/${now}_Log.txt"
echo "Script start: $start_m:$start_s" >> $LOGFILE 2>%1

source_dir="/volume1/syncthing/audio/audio_recordings/"
backup_dir="/volume1/backups/audio/audio_recordings"
num_backups_to_keep=2

echo "Compressing and backing up audio_recordings audio at ${source_dir}" >> $LOGFILE 2>%1
cd ${source_dir}
#tar -czf - Application\ Support/ -P | pv -s $(du -sb Application\ Support/ | awk '{print $1}') | gzip > $backup_dir/wiki_backup_$now.tar.gz
#tar -czf $backup_dir/${now}_audio_recordings.tar.gz "${source_dir}"
7z a $backup_dir/${now}_audio_recordings.7z "${source_dir}" -p"password" -mx=1 -t7z -m0=lzma2 
#7z a $backup_dir/${now}_audio_recordings.7z "${source_dir}" -p"password"


# Get the number of files in the backup directory
num_files=`ls $backup_dir/*_audio_recordings.7z | wc -l`
echo "Number of files in directory: $num_files" >> $LOGFILE 2>%1
# Get the full path of the oldest file in the directory
oldest_file=`ls -t $backup_dir/*_audio_recordings.7z | tail -1`
...
if [[ $? -eq 0 ]]; then
  echo "audio_recordings Backup completed in $runtime" >> $LOGFILE 2>%1
else
  echo "audio_recordings Backup failed. See log for more details." >> $LOGFILE 2>%1
fi
