...
LOGFILE="/volume1/backups/audio/audio_patches/${now}_Log.txt"
echo "Script start: $start_m:$start_s" >> $LOGFILE 2>%1

source_dir="/volume1/syncthing/audio/audio_patches/"
backup_dir="/volume1/backups/audio/audio_patches"
...
7z a $backup_dir/${now}_patches_backup.7z "${source_dir}" -p"password" -mx=1 -t7z -m0=lzma2 
#7z a $backup_dir/${now}_patches_backup.7z "${source_dir}" -p"password"


# Get the number of files in the backup directory
num_files=`ls $backup_dir/*_patches_backup.7z | wc -l`
echo "Number of files in directory: $num_files" >> $LOGFILE 2>%1
# Get the full path of the oldest file in the directory
oldest_file=`ls -t $backup_dir/*_patches_backup.7z | tail -1`
